class Solution {
public:
    string createLine(vector<string> &words,int mxLen){
        int wordLen = 0;
        for(auto &x : words){
            wordLen += (x.length());
        }
        int totalSpace = mxLen - wordLen;
        if(words.size() == 1){
    ...
    string createEndLine(vector<string> &words,int mxLen){
        int wordLen = 0;
        for(auto &x : words){
            wordLen += (x.length());
        }
    ...
    vector<string> fullJustify(vector<string>& words, int maxWidth) {
        ...
        vector<string> ans;
        for(auto &x : res){
            ans.push_back(createLine(x,mxLine));
        }
        ans.pop_back();
        ans.push_back(createEndLine(res.back(),mxLine));
        return ans;
